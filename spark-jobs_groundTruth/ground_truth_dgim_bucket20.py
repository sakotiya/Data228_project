...
def load_data(spark):
        ...
        try:
            ...
            if 'passenger_count' not in df.columns:
                 if 'Passenger_Count' in df.columns:
                     df = df.withColumn('passenger_count', col('Passenger_Count'))
                 else:
                     print("  ⚠️  No passenger_count column found - using 0")
                     df = df.withColumn('passenger_count', lit(0))
    ...
    df_all = df_all.drop('payment_type')  # In case it exists
...
def main():
    ...
    WINDOW_SIZE = 10000  # Same as DGIM simulator
    ...
    try:
        ...
        results = compute_dgim_ground_truth(spark, df, OUTPUT_PATH, total, WINDOW_SIZE)

        print("\n✅ DGIM ground truth complete!")
    finally:
        spark.stop()
